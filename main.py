from scheduler import can_assign_swap
...
def approve_swap_request(request_id):
    ...
    try:
        with sql.connect("ShiftSyncDB.db") as con:
            ...
            cur.execute("""
                SELECT EmployeeId, ShiftDate, StartTime, EndTime
                FROM Shifts
                WHERE ShiftId=?
            """, (shift_id,))
            ...
            # 2.5) Scheduling logic check
            shift_data = {
                'date': shift_row[1],
                'start': shift_row[2],
                'end': shift_row[3]
            }
            print("APPROVER:", approver_id)
            print("SHIFT DATA:", shift_data)
            if not can_assign_swap(approver_id, shift_data, con):
                ...
                return redirect(url_for('dashboard'))                        
            # 3a) Find the approver's shift on the same date (to give to the requester)
            cur.execute("""
                SELECT ShiftId FROM Shifts
                WHERE EmployeeId=? AND ShiftDate=?
            """, (approver_id, shift_row[1]))
            approver_shift_row = cur.fetchone()

            # 3b) Give the requester's shift to the approver
            cur.execute("""
                UPDATE Shifts
                SET EmployeeId=?
                WHERE ShiftId=?
            ...
            # 3c) Give the approver's shift to the requester (if one exists)
            if approver_shift_row:
                cur.execute("""
                    UPDATE Shifts
                    SET EmployeeId=?
                    WHERE ShiftId=?
                """, (requester_id, approver_shift_row[0]))
        ...
        print("approve_swap_request error:", e)
